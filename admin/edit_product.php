...
$pid = trim(intval($_GET['id']));
$db->query("SELECT * FROM `products` WHERE `id` = '%d'", $pid);
$num = $db->num_rows();
$pdata = $db->getdata();
if(!$num || $num == 0)
{
...
if(isset($_POST['product_name']))
{
 ...
 $db->query("SELECT `id` FROM `products` WHERE `product_name` = '%s' AND `id` != '%d'", $pname, $id);
 $num = $db->num_rows();
 if($num > 0)
 {
   http_response_code(400);
 ...
 else
 {
  ...
  {
   $product = $text_data;
   $db->query("UPDATE `products` SET `product` = '%s' WHERE `id` = '%d'", $product, $id);
  }
  ...
  else
  {
   ...
   if(!empty($_FILES["cover_local"]['name']))
   {
   ...
   @unlink($upDir . $pdata['product_image']);
    $db->query("UPDATE `products` SET `product_image` = '%s' WHERE `id` = '%d'", $cfilename, $id);
   }
    if(!$duration_in || empty($duration_in) || empty($duration))
    {
    $db->query("UPDATE `products` SET `expiry_duration` = NULL, `expiry_duration_in` = NULL WHERE `id` = '%d'", $id);
    }
    else
    {
    $db->query("UPDATE `products` SET `expiry_duration` = '%d', `expiry_duration_in` = '%s' WHERE `id` = '%d'", $duration, $duration_in, $id);
    }

    // Update all fields including automation
    $db->query("UPDATE `products` SET
        ...
        `renewal_plans` = '%s'
        WHERE `id` = '%d'",
        $pname, $product_type, $price, $is_require_data, $rdata,
        $description, $ldescription, $pslug, $show,
   ...
   echo json_encode(array("message" => "Product Details Successfully Updated", "redirect" => "edit_product.php?id={$id}" ));
   exit();
 }
 exit();
}
...
<form action="edit_product.php?id=<?= trim(intval($_GET['id'])); ?>" method="post" enctype="multipart/form-data" class="ajaxform_with_redirect">
<div class="col-lg-12">
<div class="card">
<div class="card-body">
...
<input type="text" class="form-control" placeholder="Enter Product Name" required="" value="<?= htmlspecialchars($pdata['product_name']); ?>" name="product_name">
</div>
</div>
<div class="transaction_fixed col-lg-6 col-md-6">
<div class="form-group">
<label>Price</label>
<input type="number" class="form-control" name="price" id="price" placeholder="Enter Product Price" value="<?= $pdata['price']; ?>">
</div>
</div>
</div>
...
<input type="text" class="form-control" placeholder="Enter slug" name="slug" value="<?= htmlspecialchars($pdata['slug']); ?>">
</div>
</div>

<div class="col-md-6">
<div class="form-group">
<label>Show/ Hide Product</label>
<select name="show" class="form-control">
<option value="1" <?= $pdata['show'] == 1 ? 'selected' : ''; ?>>Keep Showing</option>
<option value="2" <?= $pdata['show'] == 2 ? 'selected' : ''; ?>>Hide</option>
</select>
</div>
</div>
</div>
...
<img src="<?php if($pdata['product_image'] && !empty($pdata['product_image'])){echo "../panel-assets/uploads/{$pdata['product_image']}";}else{echo "../panel-assets/img/img/placeholder.png";} ?>" alt="" id="cover_photo_parent_preview" class="rounded-0" width="200">
</div>
<div class="form-group">
...
<input type="hidden" name="p_conver" id="cover" class="form-control" required>
</div>
</div>
...
<div class="row">
<div class="col-lg-6 col-md-6 col-sm-6">
<input type="number" class="form-control" name="duration" id="duration" placeholder="1" value="<?= $pdata['expiry_duration']; ?>">
</div>
<div class="col-lg-6 col-md-6 col-sm-6">
<select name="duration_in" class="form-control">
<option value="">No Expiry</option>
<option value="days" <?php if($pdata['expiry_duration_in'] == "days"){echo "selected";} ?>>Days</option>
<option value="months" <?php if($pdata['expiry_duration_in'] == "months"){echo "selected";} ?>>Months</option>
<option value="years" <?php if($pdata['expiry_duration_in'] == "years"){echo "selected";} ?>>Years</option>
</select>
</div>
</div>
</div>
...
<textarea name="description" class="form-control" style="height: 324px;" id="description" placeholder="Enter Product Short Description"><?= htmlspecialchars($pdata['product_description']); ?></textarea>
</div>
</div>
...
<textarea name="ldescription" id="ldescription" class="summernote" placeholder="Enter Product Long Description"><?= $pdata['long_product_description']; ?></textarea>
</div>
</div>
</div>
...
<option value="1" <?php if($pdata['product_type'] == "1"){echo "selected";} ?>>File (Downlaodable File)</option>
<option value="2" <?php if($pdata['product_type'] == "2"){echo "selected";} ?>>Text/ License</option>
</select>
</div>
</div>
<div class="transaction_fixed col-lg-6 col-md-6">
...
<option value="0" <?php if($pdata['is_require_data'] == "0"){echo "selected";} ?>>No</option>
<option value="1" <?php if($pdata['is_require_data'] == "1"){echo "selected";} ?>>Yes</option>
</select>
</div>
</div>

<div class="transaction_fixed col-lg-6 col-md-6 fileselect <?php if($pdata['product_type'] != "1"){echo "hide";} ?>">
<div class="form-group">
<label>Select File (<font color="blue">Selected File: <?= substr(basename($pdata['product']), 11); ?></font>)</label>
<input type="file" class="form-control" name="product" id="product">
</div>
</div>

<div class="transaction_fixed col-lg-6 col-md-6 text textadd <?php if($pdata['product_type'] == "1"){echo "hide";} ?>">
<div class="form-group">
...
<textarea class="form-control" id="text-data" name="text-data"><?php if($pdata['product_type'] != 1){ echo htmlspecialchars($pdata['product']);} ?></textarea>
</div>
</div>

<div class="form-group field_wrapper requiredata <?php if($pdata['is_require_data'] != 1){echo "hide";} ?>">
<div class="row">
<div class="col-md-5">
...
<?php
$rdatas = $pdata['require_data'];
$rdatas = explode('&&', $rdatas);
foreach($rdatas as $key => $rdata)
{
$rdata = explode('<|>', $rdata);
$label = $rdata[1] ?? '';
$type = $rdata[2] ?? 'text';
...
<input type="text" data-key="<?= $key; ?>" class="form-control" name="inputs[<?= $key; ?>][label]" value="<?= htmlspecialchars($label); ?>" placeholder="Label here">
</div>
        ...
        <div class="col-md-4">
            <div class="form-group">
                ...
                <select name="activation_type" id="activation_type" class="form-control">
                    <option value="manual" <?= ($pdata['activation_type'] ?? '') == 'manual' ? 'selected' : ''; ?>>Manual (Admin Approval Required)</option>
                    <option value="instant" <?= ($pdata['activation_type'] ?? '') == 'instant' ? 'selected' : ''; ?>>Instant (Immediate Access)</option>
                    <option value="api" <?= ($pdata['activation_type'] ?? '') == 'api' ? 'selected' : ''; ?>>API (External API Activation)</option>
                    <option value="none" <?= ($pdata['activation_type'] ?? '') == 'none' ? 'selected' : ''; ?>>None (No Activation Needed)</option>
                </select>
        ...
        <div class="col-md-4">
            <div class="form-group">
                ...
                <select name="is_free" id="is_free" class="form-control">
                    <option value="0" <?= ($pdata['is_free'] ?? 0) == 0 ? 'selected' : ''; ?>>No</option>
                    <option value="1" <?= ($pdata['is_free'] ?? 0) == 1 ? 'selected' : ''; ?>>Yes (No Payment Required)</option>
                </select>
            </div>
        </div>
        <div class="col-md-4">
            <div class="form-group">
                ...
                <select name="auto_approve" id="auto_approve" class="form-control">
                    <option value="0" <?= ($pdata['auto_approve'] ?? 0) == 0 ? 'selected' : ''; ?>>No (Wait for Admin)</option>
                    <option value="1" <?= ($pdata['auto_approve'] ?? 0) == 1 ? 'selected' : ''; ?>>Yes (Instant Approval)</option>
                </select>
            </div>
        </div>
    </div>

    <div id="api-config" class="api-config-section" style="<?= ($pdata['activation_type'] ?? '') == 'api' ? '' : 'display: none;'; ?>">
        <hr>
        ...
        <div class="form-row">
            <div class="col-md-8">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="activation_api_url" value="<?= htmlspecialchars($pdata['activation_api_url'] ?? ''); ?>" placeholder="https://api.example.com/activate">
            ...
            <div class="col-md-4">
                <div class="form-group">
                    ...
                    <select name="activation_api_method" class="form-control">
                        <option value="GET" <?= ($pdata['activation_api_method'] ?? 'GET') == 'GET' ? 'selected' : ''; ?>>GET</option>
                        <option value="POST" <?= ($pdata['activation_api_method'] ?? 'GET') == 'POST' ? 'selected' : ''; ?>>POST</option>
                    </select>
                </div>
            </div>
        </div>
        ...
        <div class="form-row">
            <div class="col-md-12">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="activation_api_key" value="<?= htmlspecialchars($pdata['activation_api_key'] ?? ''); ?>" placeholder="your-api-key-here">
        ...
        <div class="form-row">
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <textarea class="form-control" name="activation_api_params" rows="5" placeholder='{"key": "{api_key}", "email": "{activation_email}"}'><?= htmlspecialchars($pdata['activation_api_params'] ?? ''); ?></textarea>
                    <div class="json-help">
            ...
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <textarea class="form-control" name="activation_response_mapping" rows="5" placeholder='{"username": "data.username", "password": "data.password"}'><?= htmlspecialchars($pdata['activation_response_mapping'] ?? ''); ?></textarea>
                    <div class="json-help">
...
Activation Date: {activation_date}"><?= htmlspecialchars($pdata['activation_instructions_template'] ?? ''); ?></textarea>
                    <div class="json-help">
        ...
        <div class="col-md-4">
            <div class="form-group">
                ...
                <select name="requires_email_validation" id="requires_email_validation" class="form-control">
                    <option value="0" <?= ($pdata['requires_email_validation'] ?? 0) == 0 ? 'selected' : ''; ?>>No</option>
                    <option value="1" <?= ($pdata['requires_email_validation'] ?? 0) == 1 ? 'selected' : ''; ?>>Yes</option>
                </select>
            </div>
        </div>
    </div>

    <div id="email-validation-config" style="<?= ($pdata['requires_email_validation'] ?? 0) == 1 ? '' : 'display: none;'; ?>">
        <hr>
        <div class="form-row">
            <div class="col-md-12">
                <div class="form-group">
                    <label>Email Validation API URL</label>
                    <input type="text" class="form-control" name="email_validation_api_url" value="<?= htmlspecialchars($pdata['email_validation_api_url'] ?? ''); ?>" placeholder="https://api.example.com/check-email">
                </div>
            </div>
        </div>
        ...
        <div class="form-row">
            <div class="col-md-12">
                <div class="form-group">
                    ...
                    <textarea class="form-control" name="email_validation_api_params" rows="3" placeholder='{"action": "check_email", "key": "{api_key}", "email": "{email}"}'><?= htmlspecialchars($pdata['email_validation_api_params'] ?? ''); ?></textarea>
        ...
        <div class="form-row">
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="email_validation_success_key" value="<?= htmlspecialchars($pdata['email_validation_success_key'] ?? ''); ?>" placeholder="isfree">
            ...
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="email_validation_success_value" value="<?= htmlspecialchars($pdata['email_validation_success_value'] ?? ''); ?>" placeholder="1">
...
<div class="automation-section">
        ...
        <div class="col-md-4">
            <div class="form-group">
                ...
                <select name="is_renewable" id="is_renewable" class="form-control">
                    <option value="0" <?= ($pdata['is_renewable'] ?? 0) == 0 ? 'selected' : ''; ?>>No</option>
                    <option value="1" <?= ($pdata['is_renewable'] ?? 0) == 1 ? 'selected' : ''; ?>>Yes</option>
                </select>
            </div>
        </div>
    </div>

    <div id="renewal-config" style="<?= ($pdata['is_renewable'] ?? 0) == 1 ? '' : 'display: none;'; ?>">
        <hr>
        <div class="form-row">
            <div class="col-md-12">
                <div class="form-group">
                    <label>Renewal API URL</label>
                    <input type="text" class="form-control" name="renewal_api_url" value="<?= htmlspecialchars($pdata['renewal_api_url'] ?? ''); ?>" placeholder="https://api.example.com/extend">
                </div>
            </div>
        </div>
    ...
    "plan": "{plan_id}",
    "amount": "{plan_price}"
}'><?= htmlspecialchars($pdata['renewal_api_params'] ?? ''); ?></textarea>
                    <div class="json-help">
                        <strong>Available placeholders:</strong><br>
...
]'><?= htmlspecialchars($pdata['renewal_plans'] ?? ''); ?></textarea>
                    <div class="json-help">
